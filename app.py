...
def render_auth_sidebar():
    ...
    with st.sidebar.expander("🔁 ลืมรหัสผ่าน?"):
        with st.form("forgot_pw_form"):
            fp_username = st.text_input("ชื่อผู้ใช้ *", key="fp_username")
            fp_pw1 = st.text_input("รหัสผ่านใหม่ *", type="password", key="fp_pw1")
            fp_pw2 = st.text_input("ยืนยันรหัสผ่านใหม่ *", type="password", key="fp_pw2")
            do_fp = st.form_submit_button("รีเซ็ตรหัสผ่าน")

        if do_fp:
            if not fp_username or not fp_pw1 or not fp_pw2:
                st.sidebar.error("กรุณากรอกชื่อผู้ใช้ และรหัสผ่านใหม่ให้ครบ")
            elif fp_pw1 != fp_pw2:
                st.sidebar.error("ยืนยันรหัสผ่านไม่ตรงกัน")
            else:
                row = run_query("SELECT id FROM staff WHERE is_active=1 AND username=?", (fp_username,), fetch=True)
                if not row:
                    st.sidebar.error("ไม่พบบัญชีผู้ใช้")
                else:
                    run_query("UPDATE staff SET password_hash=? WHERE id=?", (hash_password(fp_pw1), row[0]["id"]))
                    st.sidebar.success("รีเซ็ตรหัสผ่านสำเร็จ! โปรดเข้าสู่ระบบด้วยรหัสผ่านใหม่")

    # ----- Register (เปิดตลอด) -----
    with st.sidebar.expander("ยังไม่มีบัญชี? สมัครใช้งาน"):
        with st.form("register_form"):
            r_first = st.text_input("ชื่อ *", key="reg_first")
            r_last = st.text_input("นามสกุล *", key="reg_last")
            r_username = st.text_input("ชื่อผู้ใช้ (ไม่ซ้ำ) *", key="reg_username")
            r_email = st.text_input("อีเมล", key="reg_email")
            r_phone = st.text_input("เบอร์โทร", key="reg_phone")
            r_role = st.selectbox("บทบาท", ["nurse", "physio", "pharmacy", "admin"], key="reg_role")
            r_pw1 = st.text_input("รหัสผ่าน *", type="password", key="reg_pw1")
            r_pw2 = st.text_input("ยืนยันรหัสผ่าน *", type="password", key="reg_pw2")
            do_reg = st.form_submit_button("สมัครใช้งาน")

        if do_reg:
            errs = []
            if not r_first or not r_last or not r_username or not r_pw1 or not r_pw2:
            ...
            if errs:
                ...
                dup_u = run_query("SELECT id FROM staff WHERE username=?", (r_username,), fetch=True)
                if dup_u:
                    st.sidebar.error("ชื่อผู้ใช้ซ้ำ")
                else:
                    full_name = f"{r_first.strip()} {r_last.strip()}"
                    pw_hash = hash_password(r_pw1)
                    run_query(
                        "INSERT INTO staff (name, role, phone, email, username, password_hash, is_active) VALUES (?,?,?,?,?,?,1)",
                        (full_name, r_role, r_phone or None, r_email or None, r_username, pw_hash),
                    )
                    st.sidebar.success("สมัครสำเร็จ! เข้าสู่ระบบได้เลย")
...
def main():
    ...
    with tabs[0]:
        ...
        if current_user().get("role") != "admin":
            ...
            with st.form("patient_form"):
                ...
                with c_w:
                    weight = st.number_input("น้ำหนัก (กก.)", min_value=0.0, step=0.1, value=float(p.get('weight') or 0) if p and p.get('weight') else 0.0)
                with c_h:
                    height = st.number_input("ส่วนสูง (ซม.)", min_value=0.0, step=0.5, value=float(p.get('height') or 0) if p and p.get('height') else 0.0)
                drug_allergy = st.text_area("ประวัติแพ้ยา", value=(p.get('drug_allergy') if p else ""))
                feeding = st.radio("การรับประทานอาหาร", ["oral","NG"], index=(0 if not p or (p.get('feeding') or 'oral')=='oral' else 1), horizontal=True)
                ward = st.text_input("วอร์ด", value=(p.get('ward') if p else ""))
                hospital = st.text_input("โรงพยาบาล", value=(p.get('hospital') if p else ""))
                opts_bg = ["A Rh+","A Rh-","AB Rh+","AB Rh-","B Rh+","B Rh-","O Rh+","O Rh-"]
    ...
    with tabs[1]:
        ...
        with st.form("nurse_form_all_in_one"):
            ...
            submit_all = st.form_submit_button('บันทึก (กลางคืน/กลางวัน)', use_container_width=True)
    ...
    with tabs[3]:
        ...
        with st.form("med_form"):
            ...
            submit_med = st.form_submit_button("บันทึกยา", disabled=not editable, use_container_width=True)
